...
class Model():
    def __init__(self):
        ...
        if config.pretrain_dis_node_emb:
            ...
            # 读取预训练节点嵌入文件
            pretrain_dis_node_emb_path = config.pretrain_dis_node_emb[0]  # Assuming only one file path is provided
            with open(pretrain_dis_node_emb_path, 'r', encoding='utf-8') as f:
                # 读取文件中的嵌入向量
                node_embeddings = [line.strip().split() for line in f.readlines()]
                node_embeddings = {int(vec[0]): [float(val) for val in vec[1:]] for vec in node_embeddings}
                # 创建一个嵌入矩阵来保存节点嵌入
                self.dis_node_embed_init = np.zeros([self.n_node, config.n_emb])
                self.gen_node_embed_init = np.zeros([self.n_node, config.n_emb])

                # 将文件中的节点嵌入赋值给 dis_node_embed_init
                for node_id, embedding in node_embeddings.items():
                    self.dis_node_embed_init[node_id] = embedding
                for node_id, embedding in node_embeddings.items():
                    self.gen_node_embed_init[node_id] = embedding
            print('[%.2f] read initial embeddings finished.' % (time.time() - t))
        #if config.pretrain_dis_node_emb == 1:
        #    t = time.time()
        #    self.dis_node_embed_init = np.load('embed/DRKG_TransE_l2_entity.npy')
        #    self.gen_node_embed_init = np.load('embed/DRKG_TransE_l2_entity.npy')
    ...
    def evaluate_link_prediction(self):
        ...
        auc, aupr = self.link_prediction.evaluate(embedding_matrix)
        info = 'auc_0=%.4f auc_50=%.4f auc_100=%.4f aupr_0=%.4f aupr_50=%.4f aupr_100=%.4f' % (auc[0], auc[1], auc[2], aupr[0], aupr[1], aupr[2])
        self.my_print(info, False, 1)
        return auc,aupr
