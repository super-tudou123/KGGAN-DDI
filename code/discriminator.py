...
class Discriminator():
    def __init__(self, n_node, node_emd_init, config):
        ...
        self.dropout_tate = config.dropout_rate


        #with tf.variable_scope('disciminator'):
        #if  len(node_emd_init) > 0:
        #if node_emd_init:
        ...
        self.dis_b_1 = tf.get_variable(name='dis_b',
                                           ...
                                           initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                                           trainable=True)
        self.dis_w_2 = tf.get_variable(name='dis_w_2',
                                           shape=[2, self.emd_dim, self.emd_dim],
                                           initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                                           trainable=True)
        self.dis_b_2 = tf.get_variable(name='dis_b_2',
                                           shape=[2, self.emd_dim],
                                           initializer=tf.contrib.layers.xavier_initializer(uniform=False),
                                           trainable=True)
        ...
        for i in range(2):
            _node_embedding_matrix.append(tf.reshape(tf.nn.embedding_lookup(self.node_embedding_matrix, i), [-1, self.emd_dim]))
            _node_embedding_matrix = [tf.nn.dropout(x, rate=0.3) for x in _node_embedding_matrix]
        ...
        self.pos_loss = 0.5*tf.reduce_mean(tf.square(pos_score - 1.0))
        ...
        for i in range(2):
            for j in range(2):
                ...
                neg
                _score = tf.matmul(node_embedding, _fake_node_embedding, transpose_b=True)
                #_neg_loss[i * 2 + j] = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(neg_score), logits=neg_score))
                _neg_loss[i * 2 + j] = 0.5*tf.reduce_mean(tf.square(neg_score))
