...
class Generator():
    def __init__(self, n_node, node_emd_init, config):
        ...
        for i in range(2):
            ...
            prob = tf.nn.sigmoid(_score[i])
            _neg_loss[i] = tf.reduce_mean(tf.square(tf.ones_like(prob) * (1.0 - config.label_smooth) - prob)+ \
                           config.lambda_gen * (tf.nn.l2_loss(self.node_embedding) + tf.nn.l2_loss(self.gen_w_1[i])))
            #_neg_loss[i] = 0.5*tf.reduce_mean(tf.square(prob-1))
